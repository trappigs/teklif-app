...
export default function ProposalContent({ availableLands, defaultSettings }: ProposalContentProps) {
  ...
  const clearOptions = (index: number) => {
    ...
    newItems[index].area = '';
    newItems[index].ada = '';
    newItems[index].parsel = '';
    newItems[index].cashPrice = 0;
    newItems[index].option1 = { price: 0, downPayment: 0, installmentCount: 0 };
    newItems[index].option2 = { price: 0, downPayment: 0, installmentCount: 0 };
    setProposalItems(newItems);
  };

  const clearOption1 = (index: number) => {
    const newItems = [...proposalItems];
    newItems[index].option1 = { price: 0, downPayment: 0, installmentCount: 0 };
    setProposalItems(newItems);
  };

  const clearOption2 = (index: number) => {
    const newItems = [...proposalItems];
    newItems[index].option2 = { price: 0, downPayment: 0, installmentCount: 0 };
    setProposalItems(newItems);
  };
  ...
  const filteredLands = availableLands.filter(land => {
    ...
    return land.title.toLocaleLowerCase('tr-TR').includes(searchLower) || formattedLoc.toLocaleLowerCase('tr-TR').includes(searchLower) || (land.price || 0).toString().includes(searchLower);
  });
      ...
      <div className="min-h-screen bg-stone-100 py-8 md:py-12 print:py-0 print:bg-white text-black">
        ...
        <div className="container mx-auto px-4 mb-8 flex flex-col md:flex-row justify-between items-center print:hidden max-w-4xl gap-4">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            >
              {isSaving ? '...' : proposalSaved ? <><Check size={20} /> Link Alındı</> : <><Save size={20} /> Link Oluştur</>}
            </button>
          </div>
        </div>

        <div className="max-w-[210mm] mx-auto bg-white shadow-2xl print:shadow-none print:w-full print:max-w-none min-h-[297mm] p-6 md:p-12 relative overflow-hidden text-black text-[13px] md:text-[15px] print:p-8">
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 relative z-10 border-b-2 border-brand/10 pb-6 gap-4 md:gap-0 print:flex-row print:items-end">
            <div>
              <div className="flex items-center mb-1">
                <img src="/logo2.png" alt="Logo" className="h-20 w-auto object-contain" />
              </div>
            </div>
                        ...
                        <table className="w-full text-left table-fixed">
                          ...
                          <tbody className="divide-y divide-stone-100 text-[10px] md:text-sm">
                            <tr className="bg-brand-light/5">
                              ...
                              <td className="py-3 px-2 md:px-4 text-right font-bold whitespace-nowrap text-brand-dark">-</td>
                            </tr>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-stone-50 p-4 rounded-lg border border-stone-100">
                      <div>
                        <h5 className="font-bold text-brand-dark text-sm mb-3 border-b border-brand/10 pb-1 flex justify-between items-center">
                          Seçenek 1 (Vadeli)
                          <button onClick={() => clearOption1(index)} className="text-[10px] text-stone-400 hover:text-orange-500 flex items-center gap-1 font-medium transition-colors">
                      ...
                      <div>
                        <h5 className="font-bold text-emerald-800 text-sm mb-3 border-b border-emerald-100 pb-1 flex justify-between items-center">
                          Seçenek 2 (Vadeli)
                          <button onClick={() => clearOption2(index)} className="text-[10px] text-stone-400 hover:text-orange-500 flex items-center gap-1 font-medium transition-colors">
                            <RefreshCw size={10} /> Temizle
                          </button>
                        </h5>
                        <div className="grid grid-cols-2 gap-2 mb-2">
